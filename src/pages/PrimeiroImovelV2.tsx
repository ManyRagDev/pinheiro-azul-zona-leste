  ...
  return (
    <style>{`
      ...
      .pi-scanline {
        ...
        animation: piScan 5.6s ease-in-out infinite;
      }
      @keyframes piScan {
      ...
      @media (prefers-reduced-motion: reduce) { .pi-scanline { animation: none; } }
    `}</style>
  );
}
    ...
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-[#06192c]/10 bg-[#f4f0e8]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {links.map((link) => (
            <Link key={link.href} to={link.href} className={`px-3 py-2 text-sm font-semibold transition hover:bg-[#06192c] hover:text-[#f4f0e8] ${link.href === "/primeiro-imovel" ? "bg-[#06192c] text-[#f4f0e8]" : "text-[#27394d]"}`}>
              {link.label}
            </Link>
          ))}
        </nav>
...
/* ─────────── dados ─────────── */
const profile: Exclude<LeadProfile, "venda_imovel"> = "primeiro_imovel";

const proofItems = [
  { icon: PiggyBank, label: "FGTS e entrada organizada", desc: "Sabemos exatamente o que você pode usar e o que falta para fechar a conta" },
  { icon: ShieldCheck, label: "Crédito sem surpresa", desc: "Entendemos sua renda antes de sugerir qualquer parcela" },
  { icon: MapPin, label: "Bairro que cabe no bolso", desc: "Itaquera, Penha, Vila Matilde — regiões com volume e acesso" },
  { icon: KeyRound, label: "Documentação acompanhada", desc: "Você não fica sozinho na papelada entre o sonho e a chave" },
];

const steps = [
  {
    num: "01",
    title: "A gente entende sua realidade",
    copy: "Quanto de FGTS? Quanto de entrada? Qual a renda familiar? A gente só aponta para imóveis que o seu bolso alcança de verdade.",
    accent: "#28c7ba",
  },
  {
    num: "02",
    title: "A gente compara bairros, não só preços",
    copy: "Itaquera, Penha, Vila Matilde — cada um tem um jeito de viver. A gente mostra o que muda na sua rotina, não só o valor do m².",
    accent: "#f3d35b",
  },
  {
    num: "03",
    title: "Você recebe um plano com nome e endereço",
    copy: "Sem lista genérica. A gente separa o que é sonho do que é possível agora — e te acompanha até a chave na mão.",
    accent: "#e43d30",
  },
];
...
const properties = [
  {
    name: "Apto Minha Casa",
    district: "Itaquera",
    signal: "entrada planejada",
    range: "até R$ 350 mil",
    narrative: "Para quem quer sair do aluguel com segurança e manter a vida perto do metrô.",
    tone: "origin" as const,
  },
  {
    name: "Lançamento popular",
    district: "Penha",
    signal: "FGTS como alavanca",
    range: "R$ 350 mil",
    narrative: "Para quem nunca comprou e precisa de um caminho claro do sonho à chave.",
    tone: "prime" as const,
  },
  {
    name: "Planta compacta",
    district: "Vila Matilde",
    signal: "mobilidade",
    range: "R$ 300 mil+",
    narrative: "Para quem trabalha no Centro e não quer gastar duas horas no trajeto.",
...
function HeroSection({ onOpenDiagnostic }: { onOpenDiagnostic: () => void }) {
  return (
    <section className="relative overflow-hidden border-b border-[#06192c]">
      <div className="absolute inset-0 pi-grid opacity-60" />
      <div className="absolute inset-0 pi-noise opacity-[0.04] mix-blend-multiply" />
      <div className="absolute inset-x-0 top-0 h-24 pi-scanline pointer-events-none" />
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_.85fr] lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-6 inline-flex border border-[#06192c] bg-[#f4f0e8] px-3 py-2 text-[10px] font-black uppercase tracking-[0.28em] text-[#06192c]">
              Da simulação à chave na mão
            </div>
            <h1 className="text-[clamp(2.4rem,5vw,4.2rem)] font-black uppercase leading-[0.95] text-[#06192c]">
              Sair do aluguel não precisa ser um salto no escuro.
            </h1>
            <p className="mt-6 text-lg leading-relaxed text-[#415064] max-w-lg">
              Comprar o primeiro imóvel é uma mistura de <strong>emoção e medo</strong>. A gente organiza seu FGTS, entrada, crédito e documentação — para que a decisão seja clara antes mesmo da primeira visita.
            </p>
    ...
    <section className="px-4 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr] lg:items-end mb-16">
          ...
          <div>
            <p className="text-[#415064] leading-relaxed">
              Cada imóvel aqui conta uma <strong>história de família</strong>. Não são ofertas — são cenários de vida que já aconteceram. O seu cenário pode ser parecido, ou completamente diferente. <strong>A gente descobre junto.</strong>
            </p>
          </div>
        </div>
        ...
        <div className="mt-12 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-[#415064]">Ainda não sabe qual cenário é o seu? A gente descobre em dois minutos de conversa.</p>
    ...
    <section className="border-t border-[#06192c]/10 bg-[#e7e0d4] px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-[#415064] leading-relaxed">
          O diagnóstico é uma conversa curta. Você conta onde está, onde quer chegar e o que não abre mão. A gente volta com opções que respeitam isso.
        </p>
...
/* ─────────── page ─────────── */
export default function PrimeiroImovelV2() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f4f0e8] text-[#06192c] selection:bg-[#e43d30] selection:text-white">
      ...
      <Footer />
      <FunnelDiagnosticModal open={diagnosticOpen} onOpenChange={setDiagnosticOpen} initialProfile={profile} sourcePage="/primeiro-imovel" />
    </div>
  );
}
