  ...
  return (
    <style>{`
      ...
      .inv-scanline {
        ...
        animation: invScan 5.6s ease-in-out infinite;
      }
      @keyframes invScan {
      ...
      @media (prefers-reduced-motion: reduce) { .inv-scanline { animation: none; } }
    `}</style>
  );
}
    ...
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-[#06192c]/10 bg-[#f4f0e8]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {links.map((link) => (
            <Link key={link.href} to={link.href} className={`px-3 py-2 text-sm font-semibold transition hover:bg-[#06192c] hover:text-[#f4f0e8] ${link.href === "/investimento" ? "bg-[#06192c] text-[#f4f0e8]" : "text-[#27394d]"}`}>
              {link.label}
            </Link>
          ))}
        </nav>
      ...
      {open && (
        <div className="grid gap-2 border-t border-[#06192c]/10 bg-[#f4f0e8] px-4 py-4 md:hidden">
          ...
          <button type="button" onClick={() => { setOpen(false); onOpenDiagnostic(); }} className="bg-[#e43d30] px-4 py-3 text-left text-sm font-black uppercase text-white">Receber diagnóstico</button>
        </div>
      )}
    </header>
  );
...
/* ─────────── dados ─────────── */
const profile: Exclude<LeadProfile, "venda_imovel"> = "investimento";

const proofItems = [
  { icon: BarChart3, label: "Liquidez que você pode conferir", desc: "Não prometemos valorização — mostramos quanto o m² gira de verdade no bairro" },
  { icon: PiggyBank, label: "Renda mensal realista", desc: "Calculamos aluguel provável, não o anúncio mais otimista do mercado" },
  { icon: TrendingUp, label: "Valorização na planta", desc: "Quando comprar na obra faz sentido e quando é melhor pronto" },
  { icon: ShieldCheck, label: "Risco de vacância mapeado", desc: "Porque investidor inteligente olha para o pior cenário, não só o melhor" },
];

const steps = [
  {
    num: "01",
    title: "A gente entende sua tese",
    copy: "Você quer renda, revenda ou valorização na planta? Cada objetivo leva para um tipo de imóvel, bairro e prazo diferente. A gente começa por aí.",
    accent: "#28c7ba",
  },
  {
    num: "02",
    title: "A gente compara números de verdade",
    copy: "Preço do m², tempo de venda, aluguel médio, vacância — tudo entra na conta para que a decisão seja racional, não emocional.",
    accent: "#f3d35b",
  },
  {
    num: "03",
    title: "Você recebe um plano com cenários",
    copy: "Sem lista genérica. A gente separa o que é especulação do que é investimento com lastro — e te liga com quem entende de números na Zona Leste.",
    accent: "#e43d30",
  },
];
...
const properties = [
  {
    name: "Studio líquido",
    district: "Tatuapé",
    signal: "aluguel rápido",
    range: "R$ 420 mil+",
    narrative: "Para quem quer renda mensal consistente em um dos eixos mais líquidos da Zona Leste.",
    tone: "origin" as const,
  },
  {
    name: "Unidade na planta",
    district: "Anália Franco",
    signal: "valorização",
    range: "R$ 700 mil+",
    narrative: "Para quem tem horizonte de médio prazo e quer aproveitar a valorização de um bairro em consolidação.",
    tone: "prime" as const,
  },
  {
    name: "Apto compacto",
    ...
    signal: "mobilidade",
    range: "R$ 500 mil",
    narrative: "Para quem quer diversificar com um ticket acessível e demanda de locação estável.",
...
function HeroSection({ onOpenDiagnostic }: { onOpenDiagnostic: () => void }) {
  return (
    <section className="relative overflow-hidden border-b border-[#06192c]">
      <div className="absolute inset-0 inv-grid opacity-60" />
      <div className="absolute inset-0 inv-noise opacity-[0.04] mix-blend-multiply" />
      <div className="absolute inset-x-0 top-0 h-24 inv-scanline pointer-events-none" />
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_.85fr] lg:items-end">
          <div className="max-w-2xl">
            <div className="mb-6 inline-flex border border-[#06192c] bg-[#f4f0e8] px-3 py-2 text-[10px] font-black uppercase tracking-[0.28em] text-[#06192c]">
              Inteligência para renda e valorização
            </div>
            <h1 className="text-[clamp(2.4rem,5vw,4.2rem)] font-black uppercase leading-[0.95] text-[#06192c]">
              Investir em imóvel pede números, não só emoção.
            </h1>
            <p className="mt-6 text-lg leading-relaxed text-[#415064] max-w-lg">
              A Zona Leste tem oportunidades reais, mas também tem armadilhas. A gente olha <strong>liquidez, aluguel, vacância e valorização</strong> — para que você compre com a tese clara e o risco medido.
            </p>
            <button type="button" onClick={onOpenDiagnostic} className="mt-8 inline-flex items-center gap-3 bg-[#06192c] px-6 py-4 text-sm font-black uppercase text-white shadow-[8px_8px_0_#e43d30] transition hover:bg-[#e43d30] hover:shadow-[8px_8px_0_#06192c]">
              Receber análise de investimento
              <ArrowRight size={18} />
            </button>
          </div>
    ...
    <section className="bg-[#06192c] px-4 py-24 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="mb-16 max-w-2xl">
          ...
          <h2 className="mt-4 text-3xl font-black uppercase leading-tight md:text-5xl">Três conversas. Um caminho mais claro.</h2>
        </div>
    ...
    <section className="px-4 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr] lg:items-end mb-16">
          <div>
            <h2 className="text-[clamp(2rem,4vw,3.8rem)] font-black uppercase leading-[0.95] text-[#06192c]">Imóveis como ponto de conversa.</h2>
          </div>
          <div>
            <p className="text-[#415064] leading-relaxed">
              Cada imóvel aqui conta uma <strong>história de investidor</strong>. Não são ofertas — são teses que já deram certo. A sua tese pode ser parecida, ou completamente diferente. <strong>A gente descobre junto.</strong>
            </p>
          </div>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {properties.map((prop) => (
            <article key={prop.name} className={`group relative border bg-white p-6 transition-all duration-300 hover:-translate-y-1 ${toneClass[prop.tone]}`}>
              <Building2 className="text-[#e43d30]" size={22} />
              <h3 className="mt-10 text-2xl font-black uppercase leading-tight text-[#06192c]">{prop.name}</h3>
              <p className="mt-2 text-sm font-bold text-[#415064]">{prop.district}</p>
              <div className="mt-5 border-l-2 border-[#28c7ba] pl-3">
                <p className="text-xs leading-relaxed text-[#5a6472] italic">{prop.narrative}</p>
              </div>
        ...
        <div className="mt-12 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-[#415064]">Ainda não sabe qual tese é a sua? A gente descobre em dois minutos de conversa.</p>
          <button type="button" onClick={onOpenDiagnostic} className="inline-flex items-center justify-center gap-2 border border-[#06192c] bg-white px-5 py-3 text-sm font-black uppercase transition hover:bg-[#06192c] hover:text-white">
    ...
    <section className="border-t border-[#06192c]/10 bg-[#e7e0d4] px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-[#415064] leading-relaxed">
          O diagnóstico é uma conversa curta. Você conta sua tese, seu prazo e o que não abre mão. A gente volta com números que fazem sentido.
        </p>
        <button type="button" onClick={onOpenDiagnostic} className="mt-8 inline-flex items-center gap-3 bg-[#e43d30] px-8 py-4 text-sm font-black uppercase text-white shadow-[6px_6px_0_#06192c] transition hover:bg-[#06192c] hover:shadow-[6px_6px_0_#e43d30]">
          Receber diagnóstico
          <ArrowRight size={18} />
        </button>
      </div>
...
/* ─────────── page ─────────── */
export default function InvestimentoV2() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f4f0e8] text-[#06192c] selection:bg-[#e43d30] selection:text-white">
      ...
      <Footer />
      <FunnelDiagnosticModal open={diagnosticOpen} onOpenChange={setDiagnosticOpen} initialProfile={profile} sourcePage="/investimento" />
    </div>
  );
}
